...
{
    public partial class F_ShipmentConfirm : Form
    {
        //メッセージ表示用クラスのインスタンス化
        ...
        public F_ShipmentConfirm()
        {
            InitializeComponent();
        }

        private void F_ShipmentConfirm_Load(object sender, EventArgs e)
        {
            // データグリッドビューの表示
            SetFormDataGridView();
        }
        ...
        private void SetDataGridView()
        {
            ...
            filteredList = filteredList.Where(x => x.ShStateFlag != 1).ToList(); //確定済みのレコードは表示しない
            ...
            dataGridViewShipment.Columns[2].Visible = false;
            dataGridViewShipment.Columns[3].Visible = false;
            dataGridViewShipment.Columns[7].Visible = false;
            dataGridViewShipment.Columns[8].Visible = false;

            //各列幅の指定
            dataGridViewShipment.Columns[0].Width = 66;
            dataGridViewShipment.Columns[1].Width = 66;
            //dataGridViewShipment.Columns[2].Width = 67;
            //dataGridViewShipment.Columns[3].Width = 67;
            dataGridViewShipment.Columns[4].Width = 66;
            dataGridViewShipment.Columns[5].Width = 105;
            dataGridViewShipment.Columns[6].Width = 115;
            //dataGridViewShipment.Columns[7].Width = 110;
            //dataGridViewShipment.Columns[8].Width = 400;
            dataGridViewShipment.Columns[9].Width = 90;
            dataGridViewShipment.Columns[10].Width = 66;
            dataGridViewShipment.Columns[11].Width = 53;
        ...
        private void GenerateDataAtSelect()
        {
            ...
            if (mShID != "")
            {
                ...
                // データの抽出
                Shipment = shipmentDataAccess.SearchShipmentConfirm(selectCondition);
                return;
            }
            else if (mOrID != "")
            {
        ...
        private void buttonConfirm_Click(object sender, EventArgs e)
        {
            ...
            var conShipment = GenerateDataAtShipment();

            //8.2.2.3 受注情報更新
            ConfirmShipment(conShipment);
        }
        ...
        private bool GetValidDataAtConfirm()
        {
            ...
            // 状態フラグの重複チェック
            if (shipmentDataAccess.CheckStateFlagExistence(textBoxShID.Text.Trim()))
            {
                //MessageBox.Show("入力された出荷IDは既に確定済みです");
                messageDsp.DspMsg("M15061");
                textBoxShID.Focus();
                return false;
            }

            // 管理フラグのチェック状態
            if (shipmentDataAccess.CheckFlagExistence(textBoxShID.Text.Trim()))
            {
                //MessageBox.Show("入力された受注IDは非表示中のため確定できません");
                messageDsp.DspMsg("M15071");
                textBoxShID.Focus();
                return false;
            }
            return true;
        ...
        //機　能   ：登録データのセット
        ///////////////////////////////
        private T_Shipment GenerateDataAtShipment()
        {
        ...
        private void ConfirmShipment(T_Shipment conShipment)
        {
            // 更新確認メッセージ
            DialogResult result = messageDsp.DspMsg("M15058");
            if (result == DialogResult.Cancel)
                return;

            // 役職情報の更新
            bool flg = shipmentDataAccess.ConfirmShipmentData(conShipment);
            if (flg == true)
                //MessageBox.Show("データを確定しました。");
                messageDsp.DspMsg("M15059");
            else
                //MessageBox.Show("データの確定に失敗しました。");
                messageDsp.DspMsg("M15060");
            ...
            {
                F_Shipment.stflg = 1;
                F_Shipment.ActiveForm.Activate();
                FormSale.F_Sale.stflg = 1;
                FormSale.F_Sale.ActiveForm.Activate();
            }
        }
        ...
        private void F_ShipmentConfirm_Activated(object sender, EventArgs e)
        {
            if (stflg == 1)
            {
                GetDataGridView();
